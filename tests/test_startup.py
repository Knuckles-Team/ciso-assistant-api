"""Import / boot smoke tests."""


def test_package_imports():
    import ciso_assistant_api

    assert hasattr(ciso_assistant_api, "Api")
    assert ciso_assistant_api._MCP_AVAILABLE is True
    assert ciso_assistant_api._AGENT_AVAILABLE is True


def test_api_composes_all_domains():
    from ciso_assistant_api.api._operation_manifest import DOMAINS
    from ciso_assistant_api.api_client import Api

    # One base class per domain plus the shared base and Api itself.
    assert len(DOMAINS) == 19
    assert callable(Api.api_request)


def test_client_constructs_with_token():
    from ciso_assistant_api.api_client import Api

    api = Api(url="https://acme.my.ciso_assistant.com", token="x")
    assert api.url == "https://acme.my.ciso_assistant.com"


def test_client_requires_credentials():
    import pytest
    from agent_utilities.core.exceptions import MissingParameterError

    from ciso_assistant_api.api_client import Api

    with pytest.raises(MissingParameterError):
        Api(url="https://acme.my.ciso_assistant.com")
